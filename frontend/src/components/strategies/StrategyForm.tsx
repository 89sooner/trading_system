...
import { createStrategyProfile } from '@/api/strategies'
  ...
  return (
    <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-3">
      ...
      <div className="space-y-1">
        ...
        <Select value={patternSetId} onValueChange={setPatternSetId}>
          <SelectTrigger>
      ...
      <div className="space-y-1">
        ...
        <Input
          id="strategyTradeQuantity"
          name="strategyTradeQuantity"
          type="number"
          step="any"
          placeholder="0.1"
        />
      ...
      <div className="space-y-1">
        ...
        <Textarea
          id="strategyLabelMap"
          name="strategyLabelMap"
          rows={3}
          placeholder={'bullish=buy\nbearish=sell'}
        />
      </div>
      <div className="space-y-1">
        <Label htmlFor="strategyThresholds">Threshold Overrides (key=value per line)</Label>
        <Textarea
          id="strategyThresholds"
          name="strategyThresholds"
          rows={3}
          placeholder={'bullish=0.7\nbearish=0.6'}
        />
      </div>
      ...
      {message && (
        <p className={`col-span-2 text-xs ${message.isError ? 'text-red-400' : 'text-green-400'}`}>
          {message.text}
        </p>
      )}
    </form>
