...
- Portfolio state persists locally via `book.json`, while backtest runs, run metadata, durable backtest jobs, dashboard equity history, live runtime session history, and incident-relevant runtime event archives persist through file-based storage or Supabase-backed PostgreSQL depending on `DATABASE_URL`.
- Backtest runs carry operator metadata such as provider, broker, strategy profile, pattern set, source, and optional notes.
- Backtest execution runs on a durable job contract claimed by either the API-owned dispatcher or the `trading_system.app.backtest_worker` CLI worker; durable queue counts, worker heartbeat, progress, and cancellation state are queryable through the API.
