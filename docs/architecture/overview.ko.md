...
- 포트폴리오 상태는 라이브 세션 재시작 복구를 위해 `book.json`에 저장되며, 백테스트 런, run metadata, 대시보드 equity 이력, live runtime session history, incident-relevant runtime event archive는 `DATABASE_URL` 유무에 따라 파일 기반 또는 Supabase PostgreSQL 기반으로 영속화된다.
